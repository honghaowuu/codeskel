...
pub fn run(args: GetArgs) -> anyhow::Result<bool> {
    ...
    let entry = if let Some(idx) = args.index {
        let rel = cache.order.get(idx).ok_or_else(|| {
            anyhow::Error::from(CodeskelError::TargetNotInTree(format!(
                "index {} (cache has {} items in order)",
                idx,
                cache.order.len()
            )))
        })?;
        cache.files.get(rel).ok_or_else(|| {
            anyhow::Error::from(CodeskelError::TargetNotInTree(rel.clone()))
        })?
    } else {
        let path = args.path.as_ref().unwrap();
        cache.files.get(path).ok_or_else(|| {
            anyhow::Error::from(CodeskelError::TargetNotInTree(path.clone()))
        })?
    };

    println!("{}", format_ok(serde_json::to_value(entry)?));
    Ok(false)
}
...
pub fn chain_order(cache: &crate::models::CacheFile, file_path: &str) -> anyhow::Result<Vec<String>> {
    let entry = cache.files.get(file_path)
        .ok_or_else(|| anyhow::Error::from(CodeskelError::TargetNotInTree(file_path.to_string())))?;
...
fn get_chain_entry(cache: &crate::models::CacheFile, file_path: &str, index: usize) -> anyhow::Result<bool> {
    ...
    let dep_path = chain.get(index)
        .ok_or_else(|| anyhow::Error::from(CodeskelError::TargetNotInTree(format!(
            "index {} for chain of '{}' (chain has {} entries)",
            index, file_path, chain.len()
        ))))?;
